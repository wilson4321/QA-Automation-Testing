...
public class OperacionMatematicaButton implements Interaction {
	...
	static void solve(String numero, Actor actor) {
		...
		actor.attemptsTo(
				...
				Click.on(Ciclos.math1.of(numero)));
	}


	@Override
	public <T extends Actor> void performAs(T actor) {
		Target operacion = Target.the("operacion").locatedBy("//div[@class='bg-white rounded-md shadow-md p-5  flex flex-col items-center my-5 w-2/5 ']//p[@class='text-center text-xl font-bold']");
	...
	public static OperacionMatematicaButton value() {
        return Tasks.instrumented(OperacionMatematicaButton.class);
    }
}
